 ...
 */
import * as Tasks from '@singleware/tasks';
 ...
 */
type DOMEventMap = {
  [name: string]: (event: DOMEvent) => void;
};

/**
 * All callbacks.
 */
const allEvents = new WeakMap<DOMElement, DOMEventMap>();

/**
 * All elements.
 */
const allJSXElements = new WeakMap<Elements.Element<any>, (DOMElement | DOMText) | (DOMElement | DOMText)[]>();

/**
 * All DOM elements.
 */
const allDOMElements = new WeakMap<DOMElement, Elements.Element<any>>();
 ...
 */
function getMouseDetails(domEvent: DOMEvent): Events.MouseDetails | undefined {
 ...
 */
function getKeyboardDetails(domEvent: DOMEvent): Events.KeyboardDetails | undefined {
 ...
 */
function getInputDetails(domEvent: DOMEvent): Events.InputDetails<any> | undefined {
 ...
 */
function dispatchEvent<D extends Details>(domEvent: DOMEvent, callback: Callback<D>) {
  const target = <object>domEvent.target;
  if (domEvent.isTrusted && Memory.hasObject(allDOMElements, target)) {
    const event = new Events.Event(domEvent.type, Memory.getObject(allDOMElements, target), <D>{
 ...
 * @param callback Event callback.
 */
function setEvent<D extends Details>(element: DOMElement, name: string, callback: Callback<D>): void {
  const eventName = name.substr(2).toLowerCase();
  const eventMap = Memory.getObject(allEvents, element);
  const eventCallback = (event: DOMEvent) => dispatchEvent(event, callback);
 ...
 * @param name Event name.
 */
function removeEvent(element: DOMElement, name: string): void {
  const eventName = name.substr(2).toLowerCase();
  const eventMap = Memory.getObject(allEvents, element);
  if (eventName in eventMap) {
    element.removeEventListener(eventName, eventMap[eventName]);
 ...
 */
function removeAttribute(domElement: DOMElement, attributeName: string): void {
 ...
 */
function setAttribute<V>(domElement: DOMElement, attributeName: string, attributeValue: V): void {
  if (attributeName.startsWith('on')) {
 ...
 */
function performAttribute(element: DOMElement, name: string, value: any): void {
  switch (name.toLowerCase()) {
    case 'perform-error':
      if ((<any>element).setCustomValidity instanceof Function) {
        new Tasks.Task(() => (<any>element).setCustomValidity(value || ''));
      }
    case 'perform-focus':
      if (value === true) {
 ...
 */
function createFromTextElement<A extends Attributes>(jsxElement: Elements.Element<Parameters<A>>): DOMText {
  const jsxParameters = <Readonly<TextParameters>>jsxElement.parameters;
 ...
 */
function createFromPlainElement<A extends Attributes>(jsxElement: Elements.Element<Parameters<A>>): DOMElement {
  const jsxParameters = <Readonly<PlainParameters<A>>>jsxElement.parameters;
 ...
 */
function createFromClassElement<A extends Attributes>(jsxElement: Elements.Element<Parameters<A>>): (DOMElement | Text)[] {
  const domChildren = [];
  for (const jsxChild of jsxElement.children) {
 ...
 */
export function createElement<A extends Attributes>(jsxElement: Elements.Element<Parameters<A>>): (DOMElement | Text)[] {
...
export function updateAttributes<A extends Attributes>(
  ...
  const domResult = <DOMElement>Memory.getObject(allJSXElements, jsxElement);
  if (domResult instanceof Array) {
  ...
  for (const name of attributesNameList) {
    if (helperAttributesList.includes(name)) {
      if (name in newAttributes) {
        performAttribute(domResult, name, newAttributes[<keyof A>name]);
      }
    } else {
      if (!(name in newAttributes)) {
        removeAttribute(domResult, name);
      } else {
        const value = newAttributes[<keyof A>name];
        if (name.startsWith('on')) {
          if (!(name in oldAttributes) || value != oldAttributes[<keyof A>name]) {
            setAttribute(domResult, name, value);
          }
        } else if (name in domResult) {
          if (!(name in oldAttributes) || value !== (<any>domResult)[name]) {
            if (value === void 0) {
              removeAttribute(domResult, name);
            } else {
              (<any>domResult)[name] = value;
            }
          }
        } else if (!(name in oldAttributes) || value != oldAttributes[<keyof A>name]) {
          setAttribute(domResult, name, value);
        }
      }
    }
